...
from src.schemas.repair_tasks import RepairTaskCreate, RepairTaskUpdate
from src.models.enums import TaskStatusEnum

class RepairTaskRepository(BaseRepository[RepairTask, RepairTaskCreate, RepairTaskUpdate]):
    ...
    async def get_full_task_graph(self, task_id: int) -> Optional[RepairTask]:
        """Достать задание со всеми этапами, бригадами и деталями (как в симуляторе)."""
        query = (
            select(self.model)
